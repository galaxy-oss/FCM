...
def strip_optimizer(f: Union[str, Path] = 'hongwai_best.pt', s: str = '') -> None:
    """
    Strip optimizer from 'f' to finalize training, optionally save as 's'.

    Args:
        f (str): file path to model to strip the optimizer from. Default is 'hongwai_best.pt'.
    ...
    def __call__(self, epoch, fitness):
        ...
        if stop:
            LOGGER.info(f'Stopping training early as no improvement observed in last {self.patience} epochs. '
                        f'Best results observed at epoch {self.best_epoch}, best model saved as hongwai_best.pt.\n'
                        f'To update EarlyStopping(patience={self.patience}) pass a new patience value, '
                        f'i.e. `patience=300` or use `patience=0` to disable EarlyStopping.')
        return stop

# if __name__ == '__main__':
#    A=get_flops(yolov8n.pt)
#    print(A)